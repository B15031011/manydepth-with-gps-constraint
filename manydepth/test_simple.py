...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default='multi', choices=('multi', 'mono'),
                        ...
                        required=False)
    parser.add_argument("--using_probability_depth",action='store_true',
                        help="If set, using weighted average method to calculate lowest_cost")
...
def test_simple(args):
    ...
    if args.using_probability_depth:
    ...
    pose_dec.load_state_dict(pose_dec_dict, strict=False)
    ...
    output_directory = os.path.join(args.image_path, 'predicted')
    print("-> Predicting on {:d} test images".format(len(paths)))

    for idx, image_path in enumerate(paths):
        if image_path.endswith("_disp_multi.jpeg") or idx==0:
            # don't try to predict disparity for a disparity image!
            continue


        input_image, original_size = load_and_preprocess_image(image_path,
                                                               resize_width=encoder_dict['width'],
                                                               resize_height=encoder_dict['height'])
        source_path=os.path.join(image_path.rsplit('\\',1)[0],'{:010d}.jpg'.format(idx-1))
        source_image, _ = load_and_preprocess_image(source_path,
                                                    resize_width=encoder_dict['width'],
                                                    resize_height=encoder_dict['height'])
        ...
        with torch.no_grad():
            ...
            lowest_cost=torch.nn.functional.interpolate(lowest_cost.unsqueeze(1),
                                      original_size, mode="bilinear", align_corners=False)[:, 0]
            lowest_cost=lowest_cost.cpu().numpy()


            # Saving numpy file

            # name_dest_npy = os.path.join(output_directory, "{:010d}_disp.npy".format(idx))
            ...
            for plot_name, toplot in (('costvol_min', lowest_cost), ('disp', sigmoid_output_resized)):
                ...
                name_dest_im = os.path.join(output_directory,
                                            "{}_{:010d}.jpeg".format(plot_name, idx))
                im.save(name_dest_im)

            print("Processed {:d} of {:d} images - saved prediction to {}".format(
                idx + 1, len(paths), output_directory))
