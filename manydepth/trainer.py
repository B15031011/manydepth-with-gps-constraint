...
class Trainer:
    def __init__(self, options):
        ...
        self.matching_ids = [0]     # [0,1,-1,-2,……] if use future frame, [0,-1,-2,……] if not use future frame
        ...
        # MODEL SETUP,修改了lowest计算方式
        if self.opt.using_probability_depth:
            print("using probability lowest cost !")
        ...
        if self.opt.using_scale_factor:
            print("add predicting scaling factor!")
            self.models["pose"] = \
                networks.PoseDecoder(self.models["pose_encoder"].num_ch_enc,
                                     num_input_features=1,
                                     num_frames_to_predict_for=2,using_scaling=True)
        else:
            self.models["pose"] = \
                networks.PoseDecoder(self.models["pose_encoder"].num_ch_enc,
                                     num_input_features=1,
        ...
        if self.opt.using_GTPose:
            print("using GTPose ground truth！ ")
            train_dataset = self.dataset(
                self.opt.data_path, train_filenames, self.opt.height, self.opt.width,
                frames_to_load, 4, is_train=True, using_GTPose=True, img_ext=img_ext)
            val_dataset = self.dataset(
                self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
                frames_to_load, 4, is_train=False, using_GTPose=True,img_ext=img_ext)
        else:
            train_dataset = self.dataset(
                self.opt.data_path, train_filenames, self.opt.height, self.opt.width,
        ...
        for scale in self.opt.scales:
            ...
            self.backproject_depth[scale] = BackprojectDepth(self.opt.batch_size, h, w) #[B, 4, H*W]
            self.backproject_depth[scale].to(self.device)

            self.project_3d[scale] = Project3D(self.opt.batch_size, h, w)#[B, H, W, 2]
    ...
    def run_epoch(self,epoch):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            outputs, losses = self.process_batch(inputs, epoch,is_train=True)
    ...
    def process_batch(self, inputs, epoch, is_train=False):
        ...
        relative_poses = torch.stack(relative_poses, 1)# [B, F, 4, 4]

        lookup_frames = [inputs[('color_aug', idx, 0)] for idx in self.matching_ids[1:]]
        lookup_frames = torch.stack(lookup_frames, 1)  # [B, F, 3, H, W]

        # apply static frame and zero cost volume augmentation
        batch_size = len(lookup_frames)
        augmentation_mask = torch.zeros([batch_size, 1, 1, 1]).to(self.device).float() #[b, 1, 1, 1]
        if is_train and not self.opt.no_matching_augmentation:
            for batch_idx in range(batch_size):
                ...
                if rand_num < 0.25:
                    ...
                    augmentation_mask[batch_idx] += 1  #标记是哪张图的lookframe被替换成了原图

                # missing cost volume augmentation -> set all poses to 0, the cost volume will
                # skip these frames
                # 以0.5的概率将cost volume置零，等效为将relative pose置零
                elif rand_num < 0.5:
                    relative_poses[batch_idx] *= 0
                    augmentation_mask[batch_idx] += 1
        outputs['augmentation_mask'] = augmentation_mask  #[b, 1, 1, 1]
        ...
        mono_losses = self.compute_losses(inputs, mono_outputs, epoch,is_multi=False)
        ...
        if not self.opt.disable_motion_masking:
            ...
            # outputs["consistency_mask"] = 1 - (outputs["consistency_mask"] *self.compute_matching_mask(outputs))

        self.generate_images_pred(inputs, outputs, is_multi=True)
        losses = self.compute_losses(inputs, outputs, epoch, is_multi=True)
        ...
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    if self.opt.using_scale_factor:
                        axisangle, translation, scale = self.models["pose"](pose_inputs)
                        outputs[("axisangle", 0, f_i)] = axisangle
                        outputs[("translation", 0, f_i)] = translation
                        outputs[("scale", 0, f_i)] = scale
                    else:
    ...
    def compute_reprojection_loss(self, pred, target):
        ...
        if self.opt.no_ssim:
            ...
            ssim_loss = self.ssim(pred, target).mean(1, True)  #保留维度
            reprojection_loss = 0.85 * ssim_loss + 0.15 * l1_loss

        return reprojection_loss #[b, 1, h, w]

    def compute_gps_scale_loss(self,inputs,outputs,is_multi=False):
        """
        input[("GPS_Pose_T",i)]:[B,3]
        outputs[("translation", 0, i)]:[B,1,3]
        """
        loss=0
        for i in self.opt.frame_ids[1:]:
            if i!="s":
                ground_T=(inputs[("GPS_Pose_T",i)]-inputs[("GPS_Pose_T",0)])
                ground_T_norm=torch.norm(ground_T,dim=1) #[B]

                pre_T=outputs[("translation", 0, i)][:,0].squeeze(1) #[B,3]

                if not pre_T.is_contiguous():
                    pre_T=pre_T.contiguous()

                if is_multi:
                    pre_T=pre_T.detach()

                pred_T_norm=torch.norm(pre_T,dim=1)

                ratio=ground_T_norm/pred_T_norm #[B]

                loss+=torch.mean((ratio-1)**2)

        return 0.5*loss

    def compute_sim3_scale_loss(self,inputs,outputs,is_multi=False):
        """
        input[("GPS_Pose_T",i)]:[B,3]
        outputs[("translation", 0, i)]:[B,1,3]
        """
        ...
        for i in self.opt.frame_ids[1:]:
            if i!="s":
                ...
                pred_T_norm = torch.norm(pre_T, dim=1) #[B]
                pre_scale=outputs[("scale", 0, i)][:,0].squeeze() #[B]

                if is_multi:
                    pre_scale=pre_scale.detach()

                ratio=ground_T_norm/pred_T_norm*pre_scale #[B]
    ...
    def compute_matching_mask(self, outputs):
        ...
        matching_depth = 1 / outputs['lowest_cost'].unsqueeze(1).to(self.device) #[B, 1, H/4, W/4]
    ...
    def compute_losses(self, inputs, outputs,epoch, is_multi=False):
        ...
        for scale in self.opt.scales:
            ...
            reprojection_losses = torch.cat(reprojection_losses, 1) #[b, ref_frames, h, w]
            ...
            if not self.opt.disable_automasking:
                ...
                identity_reprojection_losses = torch.cat(identity_reprojection_losses, 1) #[b, ref_frames, h, w]
            ...
            # find minimum losses from [reprojection, identity] , 继承monodepth2的automasking, 1为好的，0为坏的
            ...
            # consistency loss to
            if is_multi:
                reprojection_loss_mask = torch.ones_like(reprojection_loss_mask) #[b, 1, h, w]
                if not self.opt.disable_motion_masking:
            ...
            if is_multi:
                ...
                # 强行使得误差较大的这些像素的深度与原始的单目框架产生的深度值一致
            ...
            mean_disp = disp.mean(2, True).mean(3, True) #[b, 1, 1, 1]
            norm_disp = disp / (mean_disp + 1e-7)
            smooth_loss = get_smooth_loss(norm_disp, color)

            losses["smoothness_loss/{}".format(scale)] = self.opt.disparity_smoothness * smooth_loss / (2 ** scale)
        ...
        if self.opt.using_GTPose:
            # gps_weight =math.exp(epoch-21)
            gps_weight =1e-3

            if self.opt.using_scale_factor:
                losses["loss_gps"] = gps_weight * self.compute_sim3_scale_loss(inputs, outputs, is_multi)
            else:
                losses["loss_gps"] = gps_weight*self.compute_gps_scale_loss(inputs, outputs, is_multi)
        else:
            losses["loss_gps"]=0
