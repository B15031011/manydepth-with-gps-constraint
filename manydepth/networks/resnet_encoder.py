...
class ResnetEncoderMatching(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, input_height, input_width,
                 ...
                 adaptive_bins=False, depth_binning='linear',using_probability_depth=False):
        ...
        self.using_probability_depth=using_probability_depth
    ...
    def compute_depth_bins(self, min_depth_bin, max_depth_bin):
        ...
        if self.is_cuda:
            self.warp_depths = self.warp_depths.cuda()#[self.num_depth_bins, 1, H, W]

    def match_features(self, current_feats, lookup_feats, relative_poses, K, invK):
        """
        current_feats: [B, 64, H/4, W/4]
        lookup_feats：[B, F, 64, H/4, W/4]
        ...
        for batch_idx in range(len(current_feats)):
            ...
            _lookup_feats = lookup_feats[batch_idx:batch_idx + 1] # [1, F, 64, H/4, W/4]
            _lookup_poses = relative_poses[batch_idx:batch_idx + 1] # [1, F, 4, 4]

            _K = K[batch_idx:batch_idx + 1] # [1, 4, 4]
            _invK = invK[batch_idx:batch_idx + 1] # [1, 4, 4]
            world_points = self.backprojector(self.warp_depths, _invK)#[self.num_depth_bins, 4, H*W]

            # loop through ref images adding to the current cost volume
            for lookup_idx in range(_lookup_feats.shape[1]):
                lookup_feat = _lookup_feats[:, lookup_idx]  # [1, 64, H/4, W/4]
                lookup_pose = _lookup_poses[:, lookup_idx] #[1, 4, 4]

                # ignore missing images,间接等于将cost volume置为0
                if lookup_pose.sum() == 0:
                    continue

                lookup_feat = lookup_feat.repeat([self.num_depth_bins, 1, 1, 1])# [self.num_depth_bins, 64, H/4, W/4]
                pix_locs = self.projector(world_points, _K, lookup_pose) #[self.num_depth_bins, H/4, W/4, 2]
                warped = F.grid_sample(lookup_feat, pix_locs, padding_mode='zeros', mode='bilinear',
                                       align_corners=True) #[self.num_depth_bins, 64, H/4, W/4]
                ...
                x_vals = (pix_locs[..., 0].detach() / 2 + 0.5) * (
                    self.matching_width - 1)  # convert from (-1, 1) to pixel values,#[self.num_depth_bins, H/4, W/4]
                y_vals = (pix_locs[..., 1].detach() / 2 + 0.5) * (self.matching_height - 1)

                edge_mask = (x_vals >= 2.0) * (x_vals <= self.matching_width - 2) * \
                            (y_vals >= 2.0) * (y_vals <= self.matching_height - 2)#edge_mask的判断是为什么？
                edge_mask = edge_mask.float() #[self.num_depth_bins, H/4, W/4]

                # masking of current image
                current_mask = torch.zeros_like(edge_mask)#[self.num_depth_bins, H/4, W/4]
                current_mask[:, 2:-2, 2:-2] = 1.0
                edge_mask = edge_mask * current_mask#[self.num_depth_bins, H/4, W/4], 将边缘和超出范围的像素位置屏蔽掉

                diffs = torch.abs(warped - current_feats[batch_idx:batch_idx + 1]).mean(
                    1) * edge_mask  #[self.num_depth_bins, H/4, W/4]

                # integrate into cost volume
                cost_volume = cost_volume + diffs      #[self.num_depth_bins, H/4, W/4]
                counts = counts + (diffs > 0).float()  #[self.num_depth_bins, H/4, W/4]
            # average over lookup images
            cost_volume = cost_volume / (counts + 1e-7)  # [self.num_depth_bins, H/4, W/4], 平均所有source feature的匹配代价

            # if some missing values for a pixel location (i.e. some depths landed outside) then
            # set to max of existing values

            #因为会将等于0的点设为最大值，会导致之后寻找这些等于零的点困难起来，所以需要把missing_mask返回
            missing_val_mask = (cost_volume == 0).float()  #[self.num_depth_bins, H/4, W/4]
        ...
        batch_cost_volume = torch.stack(batch_cost_volume, 0)#[B, self.num_depth_bins, H/4, W/4]
        cost_volume_masks = torch.stack(cost_volume_masks, 0)#[B, self.num_depth_bins, H/4, W/4]

        return batch_cost_volume, cost_volume_masks  #[B, self.num_depth_bins, H/4, W/4]

    def feature_extraction(self, image, return_all_feats=False):
        """ Run feature extraction on an image - first 2 blocks of ResNet"""

        image = (image - 0.45) / 0.225  # imagenet normalisation,[b, 3, h, w ]
    ...
    def indices_to_disparity(self, indices):
        ...
        depth = self.depth_bins[indices.reshape(-1).cpu()] #将indices展开成一列后，在depth_bins中找到对应位置的深度值
        disp = 1 / depth.reshape((batch, height, width))
        return disp #[B, H/4, W/4]

    def probability_to_disparity(self,pro_volume):
        """
        convert probability volume into 1/depth
        pro_volume: [B, self.num_depth_bins, H/4, W/4]
        """
        b,_,h,w = pro_volume.shape
        depth_volume = self.depth_bins.unsqueeze(0)
        depth_volume = depth_volume.unsqueeze(-1)
        depth_volume = depth_volume.unsqueeze(-1).repeat([b,1,h,w]).cuda()

        depth=torch.mul(pro_volume,depth_volume).sum(1).cpu()
        disp = 1/depth
    ...
    def compute_confidence_mask(self, cost_volume, num_bins_threshold=None):
        ...
        confidence_mask = ((cost_volume > 0).sum(1) == num_bins_threshold).float() #可信的像素点因该在每一个深度代价层都有代价

        return confidence_mask  #[B, H/4, W/4]
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,
        ...
        current_feats = self.features[-1] # [B, 64, H/4, W/4]
        ...
        with torch.no_grad():
            ...
            batch_size, num_frames, chns, height, width = lookup_images.shape #lookup_images：[B, F, C, H, W]
            ...
            lookup_feats = self.feature_extraction(lookup_images,
                                                   return_all_feats=False) # [B*F, 64, H/4, W/4]
            _, chns, height, width = lookup_feats.shape
            lookup_feats = lookup_feats.reshape(batch_size, num_frames, chns, height, width)# [B, F, 64, H/4, W/4]

            # warp features to find cost volume
            cost_volume, missing_mask = \
                self.match_features(current_feats, lookup_feats, poses, K, invK)  #[B, self.num_depth_bins, H/4, W/4]
            confidence_mask = self.compute_confidence_mask(cost_volume.detach() *
                                                           (1 - missing_mask.detach())) #将设为最大值的cost volume又设为零，方便confidence_mask的判断

        # for visualisation - ignore 0s in cost volume for minimum
        viz_cost_vol = cost_volume.clone().detach() # [B, self.num_depth_bins, H/4, W/4]


        if self.using_probability_depth==True:
            max_cost=viz_cost_vol.max(1)[0] # [B, H/4, W/4]
            zero_mask=(viz_cost_vol==0).float() # [B, self.num_depth_bins, H/4, W/4]
            viz_cost_vol=viz_cost_vol*(1-zero_mask)+max_cost.unsqueeze(1)*zero_mask
            inv_viz_cost_vol=max_cost.unsqueeze(1)-viz_cost_vol # [B, self.num_depth_bins, H/4, W/4]
            prob_volume=F.softmax(inv_viz_cost_vol,dim=1) # [B, self.num_depth_bins, H/4, W/4]
            lowest_cost=self.probability_to_disparity(prob_volume)
        else:
            viz_cost_vol[viz_cost_vol == 0] = 100
            mins, argmin = torch.min(viz_cost_vol, 1)  # [B, H/4, W/4]
            lowest_cost = self.indices_to_disparity(argmin) # [B, H/4, W/4]

        # mask the cost volume based on the confidence
        cost_volume *= confidence_mask.unsqueeze(1) # [B, self.num_depth_bins, H/4, W/4]
