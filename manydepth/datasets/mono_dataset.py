...
class MonoDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.using_GTPose=using_GTPose
        ...
        self.load_Pose_T = self.check_GpsPose_T()
    ...
    def preprocess(self, inputs, color_aug):
        ...
        same augmentation.
        """
        for k in list(inputs):#list(inputs)返回字典键值的列表
            if "color" in k:
                n, im, i = k
                for i in range(self.num_scales):
    ...
    def __getitem__(self, index):
        ...
        Keys in the dictionary are either strings or tuples:
            ...
            "GPS_Pose_T"                            for ground truth gps position
        ...
        if self.using_GTPose and self.load_Pose_T:
            for i in self.frame_idxs:
                GPS_Pose_T=self.get_pose_T(folder,frame_index+i)
                inputs[("GPS_Pose_T",i)]=torch.from_numpy(GPS_Pose_T.astype(np.float32))
