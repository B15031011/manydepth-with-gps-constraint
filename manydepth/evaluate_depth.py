...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.eval_teacher:
            ...
            if opt.using_probability_depth:
            ...
            if opt.using_scale_factor:
                print("using scale factor !")
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                if opt.eval_teacher:
                    ...
                    if opt.using_scale_factor:
                        scale_shape = (opt.batch_size)
                        average_scale = torch.zeros(scale_shape, dtype=torch.float).cuda()
                    ...
                    for fi in frames_to_load[1:]:
                        if fi < 0:
                            ...
                            if opt.using_scale_factor:
                                axisangle, translation, scales = pose_dec(pose_inputs)
                                average_scale = average_scale + scales[:,0].squeeze()
                            else:
                    ...
                    if opt.using_scale_factor:
                        average_scale=average_scale/opt.num_matching_frames
                        match_scales.append(average_scale.cpu().numpy())
        ...
        if opt.using_scale_factor:
            match_scales=np.concatenate(match_scales)
    ...
    ratio_output_path=opt.load_weights_folder.rsplit('/',3)[0]

    if opt.using_scale_factor:
        np.save(os.path.join(ratio_output_path,"scales.npy"),match_scales)
        print("save scales in results !")
    np.save(os.path.join(ratio_output_path,'ratios.npy'), ratios)
    print("save ratios in results !")
